...
## 11. Relay Lifecycle & Validation (v0.0.3 Final)
| Step | Action | Expected Result | Pass/Fail |
| :--- | :--- | :--- | :--- |
| 11.1 | **Relay Start** | Click `Activate Relay` in the dock. | Action result shows `queued` then terminal `completed`. UI shows `PROVISIONING` then `ACTIVE`. | |
| 11.2 | **Relay Active Check** | Observe relay section while active. | `Region`, `Latency`, and `Uptime` are populated from Rust core. | |
| 11.3 | **Relay Stop** | Click `Deactivate` in the dock. | Action result shows `queued` then terminal `completed`. UI shows `IDLE`. | |
| 11.4 | **JWT Expiry** | Trigger relay action with expired token. | Terminal `failed` with `api error 401: unauthorized` (confirming full round-trip to Aegis API). | |
| 11.5 | **Direct Intake Self-Test** | Run `run-strict-cycle.ps1 -SelfTestDirectPluginIntake`. | Plugin bypasses JS bridge; validates core IPC and backend API logic directly. | |
