...
```bash
scp -i ~/.ssh/id_server_new aegis-control-plane/migrations/004_add_provision_step.sql ec2-user@52.13.2.122:/tmp/
ssh -i ~/.ssh/id_server_new ec2-user@52.13.2.122 "sudo -u postgres psql aegis -f /tmp/004_add_provision_step.sql"
```
...
```bash
ssh -i ~/.ssh/id_server_new ec2-user@52.13.2.122 "sudo -u postgres psql aegis -c \"\\d sessions\" | grep provision_step"
```
...
```bash
scp -i ~/.ssh/id_server_new /tmp/aegis-api-linux ec2-user@52.13.2.122:/tmp/aegis-api-linux
ssh -i ~/.ssh/id_server_new ec2-user@52.13.2.122 "sudo systemctl stop aegis-api && sudo cp /tmp/aegis-api-linux /opt/aegis/bin/aegis-api && sudo systemctl start aegis-api && sleep 2 && curl -s http://localhost:8080/healthz"
```
